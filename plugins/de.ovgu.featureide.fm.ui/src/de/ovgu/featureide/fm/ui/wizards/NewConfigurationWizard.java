...
public class NewConfigurationWizard extends Wizard implements INewWizard {
	...
	public boolean performFinish() {
		...
		try {
			configuration = ConfigFormatManager.getFactory(fmPath.toString(), format).createConfiguration();
		} catch (NoSuchExtensionException e) {
			Logger.logError(e);
			configuration = ConfigFormatManager.getEmptyConfiguration();
		}
		configuration.createDefaultValues("");
		FileHandler.save(fmPath, configuration, format);
